...
function getStageGuide(stepKey, stepLevel, who, essayNA) {
  ...
  if (stepKey === "jd") {
    if (stepLevel === 0) return {
      ...
      whenToAskHelp: "현직자 인터뷰가 거절되거나 자료를 모았는데도 '직무 일상이 그려지지 않을 때'는, 직무가 너무 광범위하거나(예: '마케팅') 본인이 보는 자료의 양이 부족한 단계일 수 있습니다."};
    if (stepLevel === 1) return {
      situation: essayNA
  ...
  if (stepKey === "exp") {
    if (level === 0) return {
      ...
      guidance: who === "new" ? "수업·동아리·아르바이트·자격증·공모전 등 5개 카테고리로 분류해 최소 15개 경험 발굴. 사소해 보여도 다 적으세요 — 가치 판단은 나중에."
        ...
        : who === "switch" ? (essayNA
          ? "전환자는 현 직무 성과를 '새 직무 자격요건의 언어'로 번역하는 게 핵심입니다. 예: 영업→마케팅 전환이면 '고객 페인 포인트 파악'을 '마케팅 타깃 분석 역량'으로. 매칭 강도(◎/○/△) 표시하면 경력기술서·면접 답변 우선순위가 자동으로 나옵니다."
          : "전환자는 현 직무 성과를 '새 직무 자격요건의 언어'로 번역하는 게 핵심입니다. 예: 영업→마케팅 전환이면 '고객 페인 포인트 파악'을 '마케팅 타깃 분석 역량'으로. 매칭 강도(◎/○/△) 표시하면 자소서·면접 우선순위가 자동으로 나옵니다.")
