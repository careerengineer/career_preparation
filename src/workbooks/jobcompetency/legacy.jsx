...
const CompetencyWorkbook = () => {
  ...
  const [currentPart, setCurrentPart] = useState(() => { try { const __d = JSON.parse(localStorage.getItem('careerengineer_jobcompetency_v1') || '{}'); return (__d.basicInfo && (__d.basicInfo.industry || __d.basicInfo.position || __d.basicInfo.company)) ? 1 : 0; } catch { return 0; } });
  ...
  const goToNextStep = () => {
    ...
    else if (currentPhase === 'round3') { if (currentPart < round3Questions.length - 1) setCurrentPart(s => s + 1); else { setFinalText(prev => (prev && prev.trim()) ? prev : generateCompetencyLetter()); setCurrentPhase('completed'); } }
    window.scrollTo(0, 0);
  };
