...
const MotivationWorkbook = () => {
  ...
  const [currentStep, setCurrentStep] = useState(() => { try { const __d = JSON.parse(localStorage.getItem('careerengineer_motivation_v1') || '{}'); return (__d.basicInfo && (__d.basicInfo.industry || __d.basicInfo.position || __d.basicInfo.company)) ? 1 : 0; } catch { return 0; } });
  ...
  const goToNextStep = () => {
    ...
    else if (currentPhase === 'round3') { if (currentStep < round3Questions.length - 1) setCurrentStep(s => s + 1); else { setFinalText(prev => (prev && prev.trim()) ? prev : genLetter()); setCurrentPhase('completed'); } }
    window.scrollTo(0, 0);
  };
  const goToPrevStep = () => {
