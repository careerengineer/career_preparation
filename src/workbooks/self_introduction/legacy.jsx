...
const SELF_INTRO_EXAMPLES = {
  ...
  Q4: '예: 이커머스 CRM 인턴에서 이탈 위험군을 RFM으로 나눠 리텐션 A/B 테스트를 운영한 경험.',
  'Q5-a': '예: 처음엔 마케팅이 \'감과 카피\'라고 생각했습니다.',
  'Q5-b': '예: 쿠폰 문구보다 \'누구에게 보내는가\'가 리텐션을 9%p 올리는 걸 보고, 데이터가 메시지보다 먼저라는 걸 깨달았습니다.',
  'Q5-c': '예: 이후 모든 캠페인을 \'타겟 정의 → 가설 → 실험\' 순서로 접근하게 됐습니다.',
  Q6: '예: 30일 리텐션 +9%p, 캠페인 ROI 1.7배 (확실한 수치만; 없으면 \'재발방지를 표준화함\'처럼 의미 중심으로)',
  Q7: '예: 데이터에서 \'다음 행동\'을 끌어내는 실행력. (\'성실함\' 같은 범용 표현 대신)',
  Q8: '예: [초기] 숫자 정리를 잘함 → [경험] A/B 테스트를 직접 설계 → [역량] 가설을 실험으로 검증해 의사결정을 바꾸는 분석력.',
  Q9: '예: 귀사는 1st party data 기반이라 신입도 실험을 직접 설계할 수 있어, \'데이터로 의사결정\'이라는 제 방향과 맞습니다. (다른 회사엔 못 쓰는 이유)',
  Q10: '예: 고객 행동 데이터를 분석해 리텐션 캠페인의 타겟·메시지를 실험으로 최적화하는 데 기여하겠습니다.',
