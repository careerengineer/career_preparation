...
const SELF_INTRO_EXAMPLES = {
  ...
  Q18: '예(30초): 안녕하세요. 이커머스 CRM 인턴에서 이탈 위험군을 RFM으로 정의하고 A/B 테스트를 설계한 경험으로, 이 직무에서 데이터로 리텐션을 키우는 데 기여할 수 있는 김지원입니다. 실제로 30일 리텐션을 9%p 올렸고, 1st party data로 실험을 직접 할 수 있는 귀사에서 바로 기여하겠습니다.',
  Q19: '예(1분): 안녕하세요. 이커머스 CRM 인턴에서 이탈 위험군을 RFM으로 정의하고 A/B 테스트를 설계한 경험으로, 이 직무에서 데이터로 리텐션을 키우는 데 기여할 수 있는 김지원입니다. 실제로 30일 리텐션 +9%p, ROI 1.7배를 만들었고, 쿠폰 문구보다 \'누구에게 보내는가\'가 성과를 가른다는 것을 데이터로 확인하며 감이 아닌 가설·실험으로 일하는 법을 익혔습니다. 입사 후에는 고객 행동 데이터를 분석해 개인화 추천과 리텐션 캠페인의 타겟·메시지를 실험으로 최적화하고, 3년 안에 리텐션 지표를 오너십 있게 운영하는 분석가로 성장하겠습니다.',
