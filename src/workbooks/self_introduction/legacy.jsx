...
// 질문별 작성 예시 (데이터/마케팅 신입 페르소나 기준) — 막힐 때 참고용
const SELF_INTRO_EXAMPLES = {
  Q1: '예: 1) 데이터 분석  2) 캠페인 기획·운영  3) 성과 측정(GA4) — 공고에서 3회 이상 반복된 단어',
  Q2: "예: '데이터 분석'은 단순 집계가 아니라 \"왜 고객이 이탈하는가\"를 숫자로 규명해 다음 행동을 정하는 일입니다.",
  Q3: '예: 잘하는 사람은 숫자에서 \'다음 행동\'을 끌어내고, 못하는 사람은 보고서로 끝냅니다.',
  Q4: '예: 2024년 하반기 이커머스 스타트업 CRM팀에서 6개월간 인턴을 했습니다. 가입 후 30일 내 이탈률이 42%로 높아 마케팅 예산이 새고 있었는데, 제가 SQL로 행동 로그를 뽑아 이탈 위험군을 RFM 5개 세그먼트로 나누고 위험군에만 쿠폰 A/B 테스트를 설계·운영했습니다. (구체적인 장면·기간·내가 한 일이 드러나야 채용담당자가 "진짜 해봤구나"를 느낍니다)',
  'Q5-a': '예: 인턴 시작 전에는 마케팅을 "좋은 카피와 감각"의 일이라고 생각했고, 데이터는 결과를 확인하는 보조 도구라고만 여겼습니다.',
  'Q5-b': '예: 그런데 똑같은 쿠폰이라도 "누구에게 보내는가"에 따라 30일 리텐션이 9%p나 갈리는 것을 실험으로 직접 확인한 순간, 메시지보다 타겟 정의가 먼저라는 걸 몸으로 깨달았습니다. (깨달음의 "순간"이 구체적일수록 진정성이 보입니다)',
  'Q5-c': '예: 그 이후로는 어떤 캠페인이든 "타겟 정의 → 가설 → 실험 → 검증" 순서로 접근하는 게 습관이 됐고, 공모전·학과 프로젝트에서도 먼저 데이터로 문제를 정의한 뒤 움직였습니다. (이 변화가 앞으로 일할 때도 이어진다는 신호)',
  Q6: '예: 실험군 30일 리텐션 +9%p, 캠페인 ROI 1.7배, 그리고 이 분석을 사내 대시보드로 정례화해 팀이 계속 쓰게 만들었습니다. (확실한 수치만; 수치가 없으면 "재발방지를 표준화해 같은 실수가 사라졌다"처럼 변화 중심으로)',
  Q7: '예: 제 장점은 "데이터에서 멈추지 않고 다음 행동까지 끌어내는 실행력"입니다. ("성실함·책임감" 같은 누구나 쓰는 표현 대신, 내 경험으로 증명되는 한 단어로)',
  Q8: '예: [초기 모습] 숫자를 깔끔히 정리하는 정도였지만 → [경험] 인턴에서 이탈 분석을 멈추지 않고 직접 A/B 테스트로 연결하면서 → [직무 역량] 가설을 실험으로 검증해 의사결정을 바꾸는 분석력으로 발전했습니다.',
  Q9: '예: 귀사는 외부 데이터가 아닌 자체 1st party data를 보유해 가입부터 결제까지 행동을 끝까지 추적할 수 있고, 신입에게도 실험 설계를 맡긴다는 점이 다른 회사와 결정적으로 다릅니다. "데이터로 의사결정을 바꾸고 싶다"는 제 방향이 가장 빨리 실현될 곳이라 지원했습니다. (회사명만 빼면 다른 곳에도 쓸 수 있는 문장이면 다시 쓰세요)',
  Q10: '예: 입사 직후에는 인턴 때 다룬 GA4·A/B 테스트 역량으로 귀사 리텐션 캠페인의 타겟·메시지를 최적화하는 업무에 바로 기여하겠습니다. 아직 부족한 SQL 기반 대용량 데이터 추출은 입사 후 사내 데이터로 매일 쿼리를 연습해 3개월 안에 끌어올려, 분석가에게 의존하지 않고 이탈 신호를 직접 정의하는 데까지 기여 범위를 넓히겠습니다. (① 직무 업무와 연결 ② 지금 역량으로 할 수 있는 일 ③ 부족한 역량을 어떻게 채우며 기여를 넓힐지)',
  Q11: '예: 김지원입니다. 이 직무에서 제가 가장 잘하는 것은 데이터로 고객의 다음 행동을 설계하는 일입니다. (추상적 정체성보다, 직무에서 바로 발휘할 강점을 첫 문장에)',
  Q12: '예: 데이터 분석 · 리텐션 A/B 테스트 +9%p · 실험 기반 실행력 · 1st party data',
  Q13: '예: (30초 키워드) + RFM 세그먼트 · ROI 1.7배 · 개인화 추천 고도화 기여',
  Q14: '예: …지향하는 김지원입니다. 이커머스 CRM 인턴에서 이탈 위험군을 RFM으로 정의했습니다.',
  Q15: '예: \'누구에게\'가 핵심임을 깨닫고 A/B 테스트를 설계해 30일 리텐션을 9%p 올렸습니다.',
  Q16: '예: 이 경험으로 숫자에서 행동을 끌어내는 실행력을 갖췄습니다.',
  Q17: '예: 이 실행력을 1st party data 환경의 귀사에서 리텐션 실험으로 발휘하고 싶습니다.',
  Q18: '예(30초): 안녕하세요, 김지원입니다. 이 직무에서 제가 가장 잘하는 것은 데이터로 고객의 다음 행동을 설계하는 일입니다. 이커머스 CRM 인턴에서 이탈 위험군을 RFM으로 정의하고 A/B 테스트를 설계해 30일 리텐션을 9%p 올렸습니다. 1st party data로 실험을 직접 할 수 있는 귀사에서 리텐션을 키우는 데 바로 기여하겠습니다.',
  Q19: '예(1분): 안녕하세요, 김지원입니다. 이 직무에서 제가 가장 잘하는 것은 데이터로 고객의 다음 행동을 설계하는 일입니다. CRM 인턴에서 이탈 위험군을 RFM으로 정의하고 A/B 테스트를 설계해 30일 리텐션 +9%p, ROI 1.7배를 만들었습니다. 특히 쿠폰 문구보다 \'누구에게 보내는가\'가 성과를 가른다는 것을 데이터로 확인하며, 감이 아닌 가설·실험으로 일하는 법을 익혔습니다. 입사 후에는 고객 행동 데이터를 분석해 개인화 추천과 리텐션 캠페인의 타겟·메시지를 실험으로 최적화하고, 3년 안에 리텐션 지표를 오너십 있게 운영하는 분석가로 성장하겠습니다.',
  Q20: '예: 소리 내어 읽고 군더더기(\'저는\', \'열심히\')를 빼고 숫자·고유명사를 남기세요. \'열심히 하겠습니다\' → \'리텐션 실험을 직접 설계하겠습니다\'.',
};
...
const STEPS = [
  ...
  },
  {
    ...
    questions: [
      ...
      { label: "Q15", question: "깨달음(Q5) → 성과(Q6)", tip: '', checkpoint: "Bad: '그리고 A/B 테스트도 해봤습니다' (나열) Good: '이 깨달음을 바탕으로 실제로 적용해본 경험이 있습니다' (인과관계)", placeholder: '', referenceQuestions: ["Q5-a", "Q5-b", "Q5-c", "Q6"], relatedWorkbooks: ['experience', 'motivation', 'jobcompetency', 'personality', 'goalachievement', 'careergoal'] },
      ...
      { label: "Q17", question: "강점(Q8) → 지원동기(Q9~10)", tip: '', checkpoint: "Bad: '귀사에 입사하고 싶습니다' (동기 없음) Good: '이 분석력을 귀사의 고객 데이터 기반 환경에서 발휘하고 싶습니다' (강점이 회사 업무로 직접 연결)", placeholder: '', referenceQuestions: ["Q8", "Q9", "Q10"], relatedWorkbooks: ['experience', 'motivation', 'jobcompetency', 'personality', 'goalachievement', 'careergoal'] },
    ],
  },
  {
    step: 7,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: SPACING.lg }}>
            {s.questions.map((q, i) => (
              <div key={q.label} style={{ borderLeft: `3px solid ${COLORS.accent2}`, paddingLeft: SPACING.md }}>
                ...
                <ExampleToggle text={SELF_INTRO_EXAMPLES[q.label] || q.placeholder} />
                <textarea
