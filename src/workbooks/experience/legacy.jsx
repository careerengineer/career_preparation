...
const GuideToggle = ({ open, onToggle, label, children }) => (
  <div style={{ marginBottom: 16 }}>
    ...
    >
      <span>{label}</span>
      <span style={{ fontSize: 14, color: '#C9A86A', flexShrink: 0 }}>{open ? '▲ 닫기' : '▼ 열기'}</span>
    </button>
    {open && (
      <div style={{
        marginTop: 8,
        background: '#F2F1EC',
        border: '1px solid #6E7A8F33',
        borderLeft: '3px solid #C9A86A',
        borderRadius: 8, padding: 16,
      }}>
        <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: 1, color: '#C9A86A', margin: 0, marginBottom: 12 }}>
          GUIDE · 작성 가이드
        </p>
        {children}
      </div>
...
const ExperienceWorkbook = () => {
  ...
  const handleSaveXlsx = async () => {
    ...
    // ═══════════════════════════════════════════════════════════
    const C = {
      NAVY:     '0E2750',  // Heritage Navy
      MID_NAVY: '1B3A6B',  // Mid Navy
      GOLD:     'C9A86A',  // Signal Gold
      STEEL:    '6E7A8F',  // Steel
      CREAM:    'F2F1EC',  // Paper Cream
      IVORY:    'FBFAF6',  // Aged Ivory
      BORDER:   'E8E5DD',  // Border Light
      LINK:     '0563C1',  // Hyperlink Blue
      WHITE:    'FFFFFF',
      STAR_S:   'EBE9DF',  // Situation (베이지)
      STAR_T:   'F2F1EC',  // Task (크림)
      STAR_A:   'FBFAF6',  // Action (아이보리)
      STAR_R:   'FFFFFF',  // Result (화이트)
    };
    // ═══════════════════════════════════════════════════════════
    // 스타일 정의
    // ═══════════════════════════════════════════════════════════
    ...
    const stl = {
      // 메인 제목 행
      title: {
        font: { ...FONT, sz: 20, bold: true, color: { rgb: C.NAVY } },
        fill: { fgColor: { rgb: C.CREAM } },
        alignment: { vertical: 'center', horizontal: 'left', indent: 1 },
        border: { bottom: { style: 'medium', color: { rgb: C.NAVY } } }
      },
      // 저작권
      copyright: {
        font: { ...FONT, sz: 9, color: { rgb: C.STEEL }, italic: true },
        fill: { fgColor: { rgb: C.IVORY } },
        alignment: { vertical: 'center', horizontal: 'left', wrapText: true, indent: 1 }
      },
      // 작성 가이드
      guide: {
        font: { ...FONT, sz: 10, color: { rgb: C.NAVY } },
        fill: { fgColor: { rgb: C.IVORY } },
        alignment: { vertical: 'top', horizontal: 'left', wrapText: true, indent: 1 },
        border: { left: { style: 'thick', color: { rgb: C.GOLD } } }
      },
      // 큰 섹션 헤더 (, , )
      sectionH: {
        font: { ...FONT, sz: 14, bold: true, color: { rgb: C.WHITE } },
        fill: { fgColor: { rgb: C.NAVY } },
        alignment: { vertical: 'center', horizontal: 'left', indent: 1 },
        border: thinBorder(C.NAVY)
      },
      // 섹션 안내문
      sectionSub: {
        font: { ...FONT, sz: 10, color: { rgb: C.STEEL }, italic: true },
        fill: { fgColor: { rgb: C.CREAM } },
        alignment: { vertical: 'center', horizontal: 'left', wrapText: true, indent: 1 }
      },
      // 도움 자료 표 헤더
      tableHeader: {
        font: { ...FONT, sz: 11, bold: true, color: { rgb: C.WHITE } },
        fill: { fgColor: { rgb: C.MID_NAVY } },
        alignment: { vertical: 'center', horizontal: 'center', wrapText: true },
        border: thinBorder(C.NAVY)
      },
      // STEP/단계 셀
      tableStep: {
        font: { ...FONT, sz: 10, bold: true, color: { rgb: C.NAVY } },
        fill: { fgColor: { rgb: C.IVORY } },
        alignment: { vertical: 'center', horizontal: 'left', wrapText: true, indent: 1 },
        border: thinBorder()
      },
      // 추천 자료 (하이퍼링크)
      tableLink: {
        font: { ...FONT, sz: 11, bold: true, color: { rgb: C.LINK }, underline: true },
      ...
      infoBox: {
        font: { ...FONT, sz: 10, color: { rgb: C.NAVY } },
        fill: { fgColor: { rgb: C.IVORY } },
        alignment: { vertical: 'center', horizontal: 'left', wrapText: true, indent: 2 },
        border: { left: { style: 'thick', color: { rgb: C.MID_NAVY } } }
      },
      // 안내 박스 클릭 링크
      infoBoxLink: {
        font: { ...FONT, sz: 11, bold: true, color: { rgb: C.LINK }, underline: true },
      ...
      // 카테고리 헤더 (학교 활동 등) — 진행 정보 포함
      catHeader: {
        font: { ...FONT, sz: 12, bold: true, color: { rgb: C.WHITE } },
        fill: { fgColor: { rgb: C.MID_NAVY } },
        alignment: { vertical: 'center', horizontal: 'left', indent: 1 },
        border: mediumBorder(C.NAVY)
      },
      // 워크북 14컬럼 헤더
      wbHeader: {
        font: { ...FONT, sz: 10, bold: true, color: { rgb: C.WHITE } },
        fill: { fgColor: { rgb: C.NAVY } },
        alignment: { vertical: 'center', horizontal: 'center', wrapText: true },
        border: thinBorder(C.NAVY)
      },
      ...
      starS: {
        font: { ...FONT, sz: 10, color: { rgb: C.NAVY } },
        fill: { fgColor: { rgb: C.STAR_S } },
        alignment: { vertical: 'top', horizontal: 'left', wrapText: true, indent: 1 },
        border: thinBorder()
      },
      starT: {
        font: { ...FONT, sz: 10, color: { rgb: C.NAVY } },
        fill: { fgColor: { rgb: C.STAR_T } },
        alignment: { vertical: 'top', horizontal: 'left', wrapText: true, indent: 1 },
        border: thinBorder()
      },
      starA: {
        font: { ...FONT, sz: 10, color: { rgb: C.NAVY } },
        fill: { fgColor: { rgb: C.STAR_A } },
        alignment: { vertical: 'top', horizontal: 'left', wrapText: true, indent: 1 },
        border: thinBorder()
      },
      starR: {
        font: { ...FONT, sz: 10, color: { rgb: C.NAVY } },
        fill: { fgColor: { rgb: C.STAR_R } },
        alignment: { vertical: 'top', horizontal: 'left', wrapText: true, indent: 1 },
        border: thinBorder()
      },
      // 역량 이름 셀
      ...
      // 점수 셀
      wbScore: {
        font: { ...FONT, sz: 11, bold: true, color: { rgb: C.MID_NAVY } },
        fill: { fgColor: { rgb: C.CREAM } },
        alignment: { vertical: 'center', horizontal: 'center' },
        border: thinBorder()
      },
    ...
    experiences.forEach(e => {
      ...
      if (matched && CATEGORY_ORDER.includes(matched)) {
        grouped[matched].push(e);
      } else if (matched) {
        if (!otherCategories[matched]) otherCategories[matched] = [];
        otherCategories[matched].push(e);
      }
    });
    // 경험 → 4행(STAR) 펼치기
    const renderExperienceAs4Rows = (e) => {
      const jc = pad4(e.job_comps);
      const cc = pad4(e.comm_comps);
      const ac = pad4(e.att_comps);
      return [
        // S 행: 기본 정보 + S + 첫 역량
        [
          e.category || '', e.period || '', e.org || '', e.role || '',
          e.summary || '', e.motivation || '',
          e.star_s ? 'S (상황): ' + e.star_s : '', e.learning || '',
          jc[0].name, jc[0].score, cc[0].name, cc[0].score, ac[0].name, ac[0].score,
        ],
        // T 행
        ['', '', '', '', '', '', e.star_t ? 'T (과제): ' + e.star_t : '', '',
    ...
    helpfulRows.forEach(h => {
      helpRowIndices.push({ row: tmplRows.length, url: h.url });
      addRow([h.step, h.name, h.why, h.status], { type: 'helpRow' });
    });
    addRow(blankRow(COL_COUNT), { type: 'gap' });
    // 전체 안내 박스
    ...
    const starRowIndices = []; // {row, starType: 'S'|'T'|'A'|'R'}
    const renderCategory = (cat, experiences) => {
      ...
      experiences.forEach((e, eIdx) => {
        const rows = renderExperienceAs4Rows(e);
        rows.forEach((r, rIdx) => {
          starRowIndices.push({ row: tmplRows.length, starType: ['S','T','A','R'][rIdx] });
          addRow(r, { type: 'expRow', starType: ['S','T','A','R'][rIdx] });
        });
        // 경험 사이 구분 (마지막 경험 제외)
        if (eIdx < experiences.length - 1) {
          addRow(blankRow(COL_COUNT), { type: 'expGap' });
        }
      });
      // 빈 작성 공간 4행 (1개 경험 분량)
      for (let i = 0; i < 4; i++) {
        starRowIndices.push({ row: tmplRows.length, starType: ['S','T','A','R'][i], blank: true });
        addRow(blankRow(COL_COUNT), { type: 'expRow', starType: ['S','T','A','R'][i], blank: true });
      }
    };
    ...
    rowMeta.forEach((meta, r) => {
      switch (meta.type) {
        case 'title':
          for (let c = 0; c < COL_COUNT; c++) apply1(r, c, stl.title);
          break;
        case 'copyright':
          for (let c = 0; c < COL_COUNT; c++) apply1(r, c, stl.copyright);
          break;
        case 'guide':
          for (let c = 0; c < COL_COUNT; c++) apply1(r, c, stl.guide);
          break;
        case 'sectionH':
          for (let c = 0; c < COL_COUNT; c++) apply1(r, c, stl.sectionH);
          break;
        case 'sectionSub':
          for (let c = 0; c < COL_COUNT; c++) apply1(r, c, stl.sectionSub);
          break;
        case 'tableHeader':
          for (let c = 0; c <= 3; c++) apply1(r, c, stl.tableHeader);
          break;
        case 'helpRow':
          apply1(r, 0, stl.tableStep);
          apply1(r, 1, stl.tableLink);
          apply1(r, 2, stl.tableText);
          apply1(r, 3, stl.tableStatus);
          break;
        case 'infoBox':
          for (let c = 0; c < COL_COUNT; c++) apply1(r, c, stl.infoBox);
          break;
        case 'infoBoxLink':
          for (let c = 0; c < COL_COUNT; c++) apply1(r, c, stl.infoBoxLink);
          break;
        case 'wbHeader':
          for (let c = 0; c < COL_COUNT; c++) apply1(r, c, stl.wbHeader);
          break;
        case 'catHeader':
          for (let c = 0; c < COL_COUNT; c++) apply1(r, c, stl.catHeader);
          break;
        case 'expRow': {
          const starStyle = { S: stl.starS, T: stl.starT, A: stl.starA, R: stl.starR }[meta.starType] || stl.wbCell;
          // 컬럼별 스타일
          // 0~5: 일반 셀 (기간/기관명/역할/주요활동/하게된이유)
          for (let c = 0; c <= 5; c++) {
            apply1(r, c, meta.blank ? stl.wbBlank : stl.wbCell);
          }
          // 6: STAR
          apply1(r, 6, meta.blank ? stl.wbBlank : starStyle);
          // 7: 배운점
          apply1(r, 7, meta.blank ? stl.wbBlank : stl.wbCell);
          // 8,10,12: 역량 이름  / 9,11,13: 점수
    ...
    helpRowIndices.forEach(({ row, url }) => {
      ...
      if (ws1[addr] && url) {
        ws1[addr].l = { Target: url, Tooltip: '클릭하여 페이지 이동' };
      }
    });
    const storesAddr = XLSX_S.utils.encode_cell({ r: storesRowIdx, c: 0 });
    if (ws1[storesAddr]) {
      ws1[storesAddr].l = { Target: MENTORING_URLS.stores, Tooltip: 'CareerEngineer 전체 상품 페이지' };
    }
    // ─────────────── 인쇄 설정 ───────────────
    ...
    apply2(0, 0, stl.title);
    apply2(0, 1, stl.title);
    apply2(1, 0, stl.copyright);
    apply2(1, 1, stl.copyright);
    [3, 8, 12].forEach(r => {
      apply2(r, 0, stl.sectionH);
      apply2(r, 1, stl.sectionH);
    });
    ws2['!merges'] = [
      { s: { r: 0, c: 0 }, e: { r: 0, c: 1 } },
      { s: { r: 1, c: 0 }, e: { r: 1, c: 1 } },
    ...
    [4, 5, 6, 9, 10, 13, 14, 15, 16].forEach(r => {
      apply2(r, 0, stl.tableStep);
      apply2(r, 1, stl.tableText);
    });
    ws2['!rows'] = [
      { hpt: 36 }, { hpt: 24 }, { hpt: 10 },
      { hpt: 30 }, { hpt: 28 }, { hpt: 28 }, { hpt: 28 }, { hpt: 10 },
      { hpt: 30 }, { hpt: 28 }, { hpt: 28 }, { hpt: 10 },
      { hpt: 30 }, { hpt: 28 }, { hpt: 28 }, { hpt: 28 }, { hpt: 28 }
    ];
    ...
    for (let c = 0; c < 5; c++) {
      apply3(0, c, stl.title);
      apply3(1, c, stl.copyright);
      apply3(3, c, stl.tableHeader);
      apply3(10, c, stl.sectionH);
    }
    for (let r = 4; r <= 8; r++) {
      apply3(r, 0, stl.wbScore);
      apply3(r, 1, stl.tableStep);
      apply3(r, 2, stl.tableText);
      apply3(r, 3, stl.tableText);
      apply3(r, 4, stl.tableText);
    }
    for (let r = 11; r <= 15; r++) {
      apply3(r, 0, stl.tableStep);
      apply3(r, 1, stl.tableText);
    }
    ws3['!merges'] = [
      { s: { r: 0, c: 0 }, e: { r: 0, c: 4 } },
      { s: { r: 1, c: 0 }, e: { r: 1, c: 4 } },
    ...
    ws3['!rows'] = [
      { hpt: 36 }, { hpt: 30 }, { hpt: 10 }, { hpt: 36 },
      { hpt: 56 }, { hpt: 56 }, { hpt: 56 }, { hpt: 56 }, { hpt: 56 },
      { hpt: 10 }, { hpt: 30 }, { hpt: 26 }, { hpt: 26 }, { hpt: 26 }, { hpt: 26 }, { hpt: 26 }
    ];
    ...
    if (experiences.some(e => e.jd_match)) {
      ...
      experiences.forEach((e, i) => {
        if (e.jd_match) jdRows.push([i + 1, e.category, e.org, e.role, e.jd_match]);
      });
      const ws4 = XLSX_S.utils.aoa_to_sheet(jdRows);
      ws4['!cols'] = [{ wch: 6 }, { wch: 24 }, { wch: 24 }, { wch: 14 }, { wch: 60 }];
      const apply4 = applyStyleToSheet(ws4);
      for (let c = 0; c < 5; c++) {
        apply4(0, c, stl.title);
        apply4(1, c, stl.copyright);
        apply4(3, c, stl.tableHeader);
      }
      for (let r = 4; r < jdRows.length; r++) {
        apply4(r, 0, stl.wbScore);
        for (let c = 1; c < 5; c++) apply4(r, c, stl.tableText);
      }
      ws4['!merges'] = [
        { s: { r: 0, c: 0 }, e: { r: 0, c: 4 } },
        { s: { r: 1, c: 0 }, e: { r: 1, c: 4 } },
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: SPACING.base, flexWrap: 'wrap' }}>
            ...
            <div style={{ position: 'relative', flex: 1, display: 'flex', justifyContent: 'center' }}>
              <button onClick={() => setShowStepNav(v => !v)} style={{ 
                ...
                STEP 2 · 경험 정리{phase === 'info' ? ' · 1. 기본 정보' : phase === 'jd' ? ' · 2. 채용공고 키워드' : phase === 'list' ? ' · 3. 경험 인벤토리' : phase === 'detail' ? ' · 3. 경험 상세 작성' : phase === 'review' ? ' · 4. 최종 검토' : phase === 'complete' ? ' · 5. 완성' : ''}
            ...
            {phase !== 'intro' && (
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                <button onClick={clearSavedData} disabled={clearedFlash} style={{ background: confirmingClear ? '#C9A86A' : clearedFlash ? '#E8F5F0' : autoSaveStatus ? '#F0F9F5' : 'transparent', color: confirmingClear ? '#fff' : clearedFlash ? '#1FA47A' : autoSaveStatus ? '#1FA47A' : '#6E7A8F', border: confirmingClear ? '1px solid #C9A86A' : clearedFlash ? '1px solid #1FA47A' : autoSaveStatus ? '1px solid #1FA47A66' : '1px solid #6E7A8F66', borderRadius: RADIUS.base, padding: '0 14px', fontSize: 11, fontWeight: 600, cursor: clearedFlash ? 'default' : 'pointer', whiteSpace: 'pre-line', fontFamily: 'inherit', lineHeight: 1.15, width: 140, height: 36, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }} title={clearedFlash ? '기록 삭제됨' : confirmingClear ? '한번 더 클릭하면 기록이 삭제됩니다' : '저장된 작성 내용 기록을 삭제 (페이지 유지)'}>
                  {confirmingClear ? '기록을 삭제\n하시겠습니까?' : clearedFlash ? '✓ 기록 삭제됨' : autoSaveStatus ? autoSaveStatus : '기록 삭제하고\n다시 작성'}
                </button>
                <button className="ce-btn" onClick={handleSaveXlsx} disabled={isSavingXlsx}
                  style={{ background: isSavingXlsx ? COLORS.sub : COLORS.green, color: COLORS.white, border: 'none', borderRadius: RADIUS.base, padding: '0 14px', fontSize: FONT.size.sm, fontWeight: FONT.weight.semibold, cursor: isSavingXlsx ? 'wait' : 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 4, opacity: isSavingXlsx ? 0.7 : 1, fontFamily: 'inherit', whiteSpace: 'nowrap', height: 36 }}>
                  {isSavingXlsx ? (
                    <>
