...
function DynamicTailQuestions({ finalAnswer }) {
  ...
  if (/팀|협업|함께|동료|조직/.test(finalAnswer)) detected.push({ q: '그 안에서 본인의 구체적인 역할과 기여는 무엇이었나요?', tip: '본인이 주도한 부분과 협업한 부분을 분리해 답변' });
  if (/데이터|분석|지표|성과/.test(finalAnswer)) detected.push({ q: '그 성과를 어떤 지표·데이터로 증명할 수 있나요?', tip: '정량 지표와 측정 방법을 구체적으로' });
  if (/리더|주도|이끌|설득|관리/.test(finalAnswer)) detected.push({ q: '이해관계가 충돌했을 때 어떻게 조율했나요?', tip: '구체적 갈등 상황과 본인의 조율 방식' });
  if (/이직|퇴사|전환/.test(finalAnswer)) detected.push({ q: '왜 지금 이 시점에 이직을 결심했나요?', tip: '현 직장 비판 대신 성장·도전 관점으로' });
...
const QUESTIONS = [
  {
    ...
    "answerStrategy": "\"이름 + 연차 + 직무 정체성\" → \"대표 성과 1~2개(수치 필수)\" → \"이 회사에 어떤 가치를 가져올지\" 3단 구조. 1분 30초~2분 분량. 신입 자기소개처럼 학력·취미·동아리 나열 절대 금지. 첫 문장에 본인의 \"한 줄 경력 정체성\"을 박아야 합니다(예: \"B2B SaaS 마케팅을 5년간 전문으로 해온 ○○입니다\").",
    ...
    "stages": [
      {
        "name": "5년",
        "questions": [
          "본인 전문 영역의 5~10년 후 깊이는 어느 정도가 되어 있을까요?"
        ]
      },
      {
        "name": "10년",
  ...
  },
  {
    ...
    "answerStrategy": "\"협업 목적·부서 → 부서별 이해관계 차이 → 본인의 조율 방법(공통 목표·정보 공유·중재) → 어려움 → 정량 결과\" 구조. ①다른 부서 비난 금지 ②\"내가 다 했다\" 금지 ③구체적 협업 방법론(주간 동기화·도구 등) 포함. 좋은 답변: \"마케팅·영업·개발 3팀이 다른 우선순위를 가지고 있었습니다. 공통 KPI를 재정의하고 격주 정렬 미팅을 도입해 합의점을 만들어냈습니다.\"",
  ...
  },
  {
    ...
    "stages": [
      ...
      },
      {
        ...
        "questions": [
          "본인 경험과 직접 매칭되는 업무는 무엇인가요?",
          "본인이 이 트렌드에 대비해 학습하고 있는 것은 무엇인가요?"
        ]
      }
    ],
    ...
    "stages": [
      {
        "name": "준비",
        "questions": [
          "레퍼런스 추천인 명단이 준비되어 있나요?",
          "각 추천인과 본인의 업무 관계는 어떠했나요?",
          "팀원 피드백은 어떻게 운영하나요?"
        ]
      }
    ],
...
const CareerInterviewWorkbook = () => {
  ...
  useEffect(() => {
    ...
    window.__CE_HOME = { fn: () => __ceHomeRef.current?.(), key: 'interview_career' };

    return () => { if (window.__CE_HOME?.key === 'interview_career') window.__CE_HOME = null; };
  ...
  const STORAGE_KEY = 'careerengineer_interview_career_v1';
  ...
  useEffect(() => {
    ...
    window.__CE_DOWNLOAD = { fn: () => __ceDlRef.current?.(), key: 'interview_career' };

    return () => { if (window.__CE_DOWNLOAD?.key === 'interview_career') window.__CE_DOWNLOAD = null; };
  ...
  const downloadFinal = async () => {
    try {
      ...
      const children = [dateP(), titleP('경 력 면 접 답변집')];
      ...
      children.push(linkP('경력 면접 준비 가이드북', 'https://www.latpeed.com/products/j3RfY'));
      ...
      children.push(linkP('이직 컨설팅', 'https://www.latpeed.com/products/LimF9'));
      children.push(new Paragraph({
      ...
      try { children.push(...buildWorkbookBackupParagraphs(docxLib, buildWorkbookPayload('interview_career', '경력직 면접', 'careerengineer_interview_career_v1'))); } catch (e) { console.warn('[interview_career] backup embed skipped:', e); }
  ...
  // ══════════════════ 인트로 ══════════════════
    ...
    <IntroPage
      ...
      helpModal={<FirstVisitModal open={showHelp} onClose={() => setShowHelp(false)} title='경력 면접 준비 워크북 사용 안내' steps={[
          ...
          <div style={{ marginBottom: SPACING.lg }}>
            ...
            {(q.template || q.example) && (
              <div style={{ ...S.boxInfo, marginBottom: SPACING.sm }}>
                ...
                {/* 인라인 참고 워크북 (가이드 PART 7-15) */}
                {q.relatedWorkbooks && <RelatedWorkbookInline ids={q.relatedWorkbooks} questionId={q.id || q.label} workbookKey="interview_career" />}
              </div>
            )}
            <textarea
              className="ce-textarea"
