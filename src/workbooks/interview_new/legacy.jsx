...
const QUESTIONS = [
  {
    ...
    "answerStrategy": "\"이름+한 줄 가치관\" → \"이를 보여주는 핵심 경험 1개(수치 포함)\" → \"회사·직무에 어떻게 기여할지\" 3단 구조. 절대 학력·취미 나열로 시작하지 마세요. 첫 문장은 \"○○대학교 ○○과 ○○입니다\" 다음 바로 본인의 한 줄 정체성을 던지세요. 분량은 350자(말로 1분 30초)를 넘기지 않습니다.",
...
const NewInterviewWorkbook = () => {
  ...
  useEffect(() => {
    ...
    window.__CE_HOME = { fn: () => __ceHomeRef.current?.(), key: 'interview_new' };

    return () => { if (window.__CE_HOME?.key === 'interview_new') window.__CE_HOME = null; };
  ...
  const STORAGE_KEY = 'careerengineer_interview_new_v1';
  ...
  useEffect(() => {
    ...
    window.__CE_DOWNLOAD = { fn: () => __ceDlRef.current?.(), key: 'interview_new' };

    return () => { if (window.__CE_DOWNLOAD?.key === 'interview_new') window.__CE_DOWNLOAD = null; };
  ...
  const downloadFinal = async () => {
    try {
      ...
      const children = [dateP(), titleP('신 입 면 접 답변집')];
      ...
      // 통합 완성본
      children.push(sectionH('통합 완성본 — 핵심 답변 정리'));
      if (finalText && finalText.trim()) {
        ...
        children.push(new Paragraph({
          children: [new TextRun({ text: '[통합 완성본이 여기에 정리됩니다.]', italic: true, size: 22, font: '맑은 고딕', color: '6E7A8F' })],
          spacing: { before: 100, after: 200, line: 380 },
          shading: { fill: 'FBFAF6' },
          border: { left: { style: BorderStyle.SINGLE, size: 24, color: 'C9A86A', space: 8 } },
          indent: { left: 240 }
      ...
      // 질문별 답변
      children.push(sectionH('질문별 답변 정리'));
      QUESTIONS.forEach(qq => {
        children.push(qHeader(qq.label, qq.title, qq.required));
        
        // 면접관이 보려는 것 (가이드)
        if (qq.interviewerWants) {
          children.push(new Paragraph({
            children: [new TextRun({ text: '면접관이 이 질문으로 확인하려는 것', bold: true, size: 20, font: '맑은 고딕', color: 'C9A86A' })],
            spacing: { before: 200, after: 60 },
            indent: { left: 240 }
          }));
          children.push(new Paragraph({
            children: [new TextRun({ text: qq.interviewerWants, size: 20, font: '맑은 고딕', color: '0E2750' })],
        ...
        // 답변 전략 (가이드)
        if (qq.answerStrategy) {
          children.push(new Paragraph({
            children: [new TextRun({ text: '답변 핵심 전략', bold: true, size: 20, font: '맑은 고딕', color: '0E2750' })],
            spacing: { before: 120, after: 60 },
            indent: { left: 240 }
          }));
          children.push(new Paragraph({
            children: [new TextRun({ text: qq.answerStrategy, size: 20, font: '맑은 고딕', color: '0E2750' })],
        ...
        children.push(new Paragraph({
          children: [new TextRun({ text: '최종 답변', bold: true, size: 22, font: '맑은 고딕', color: '0E2750' })],
          spacing: { before: 280, after: 80 },
          border: { left: { style: BorderStyle.SINGLE, size: 24, color: '0E2750', space: 8 } },
          indent: { left: 240 }
        }));
        if (finalA && finalA.trim()) children.push(finalAnsP(finalA, true));
        else children.push(finalAnsP('[최종 답변 작성 전]', false));
        
        // 꼬리질문
        qq.tails.forEach((t, ti) => {
          children.push(labelP(`[꼬리질문] ${t.q}`));
      ...
      children.push(linkP('면접 유형별 답변 전략', 'https://www.latpeed.com/products/O-KKc'));
      ...
      try { children.push(...buildWorkbookBackupParagraphs(docxLib, buildWorkbookPayload('interview_new', '신입 면접', 'careerengineer_interview_new_v1'))); } catch (e) { console.warn('[interview_new] backup embed skipped:', e); }
      ...
      a.href = url;
      a.download = `신입 면접_${(basicInfo.company || '미입력').replace(/[^a-zA-Z0-9가-힣\s]/g, '_')}_${today}.docx`;
  ...
  // ══════════════════ 인트로 ══════════════════
    ...
    <IntroPage
      ...
      helpModal={<FirstVisitModal open={showHelp} onClose={() => setShowHelp(false)} title='신입 면접 준비 워크북 사용 안내' steps={[
